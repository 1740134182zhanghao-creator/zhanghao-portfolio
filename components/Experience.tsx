...
export function Experience() {
  return (
    <MotionSection id="experience" className="mx-auto max-w-6xl px-6 py-32">
        ...
        <GlassCard className="rounded-[3rem] p-7 md:p-12">
          <div className="grid gap-10 md:grid-cols-[0.36fr_1fr]">
            ...
            <ul className="grid gap-3 border-l border-white/80 pl-6">
              {experience.points.map((point) => <li key={point} className="rounded-[1.4rem] border border-white/55 bg-white/22 px-5 py-4 leading-8 text-slate-600 shadow-sm backdrop-blur-xl">{point}</li>)}
            </ul>
          </div>
        </GlassCard>
      </Reveal>
