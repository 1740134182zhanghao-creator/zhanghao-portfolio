...
import { Reveal, sectionVariants } from "./Motion";
...
export function Hero() {
  ...
  return (
    <motion.section ref={ref} id="hero" variants={reduce ? undefined : sectionVariants} initial={reduce ? false : "hidden"} whileInView={reduce ? undefined : "show"} viewport={{ once: false, amount: 0.28 }} className="mx-auto grid min-h-screen max-w-7xl items-center gap-12 px-6 pb-20 pt-36 md:grid-cols-[0.92fr_1.08fr] lg:gap-16">
      <Reveal>
        <div className="max-w-2xl">
          <p className="mb-6 w-fit rounded-full border border-white/80 bg-white/26 px-4 py-2 text-sm text-slate-600 shadow-sm backdrop-blur-2xl">服务市场：美国 · 欧洲 · 全球</p>
          ...
          <div className="mt-10 flex flex-col gap-3 sm:flex-row">
            ...
            <a href="#contact" className="rounded-full border border-white/80 bg-white/28 px-8 py-4 text-center font-medium text-slate-900 shadow-sm backdrop-blur-2xl transition hover:-translate-y-1 hover:bg-white/24 focus:outline-none focus:ring-2 focus:ring-slate-400">下载个人简历</a>
          </div>
        </div>
      </Reveal>
      ...
      <motion.div style={{ y: mediaY }} initial={{ opacity: 0, scale: 0.96, filter: "blur(14px)" }} animate={{ opacity: 1, scale: 1, filter: "blur(0px)" }} transition={{ delay: 0.22, duration: 1, ease: [0.16, 1, 0.3, 1] }} className="glass-card shine relative aspect-[4/5] overflow-hidden rounded-[3rem] p-4 md:aspect-[5/6] lg:aspect-[4/5]">
        ...
        <div className="relative flex h-full flex-col overflow-hidden rounded-[2.35rem] border border-white/62 bg-[linear-gradient(145deg,rgba(255,255,255,.72),rgba(231,240,247,.42))] p-5 shadow-inner">
          <div className="relative min-h-0 flex-1 overflow-hidden rounded-[1.9rem] bg-[radial-gradient(circle_at_24%_18%,rgba(255,255,255,.96),transparent_24%),radial-gradient(circle_at_78%_70%,rgba(141,199,255,.34),transparent_32%),linear-gradient(135deg,#eef7ff,#f9f6ff_48%,#dbe8f2)]">
            ...
            <div className="absolute left-6 top-6 rounded-full border border-white/70 bg-white/24 px-4 py-2 text-sm text-slate-600 backdrop-blur-2xl">视频占位</div>
            <div className="absolute inset-x-8 bottom-8 rounded-[2rem] border border-white/70 bg-white/28 p-6 text-center shadow-glass backdrop-blur-2xl">
              <span className="mx-auto mb-5 grid size-16 place-items-center rounded-full bg-white/36 text-2xl text-slate-950 shadow-glass">▶</span>
