...
export function Projects() {
  return (
    <MotionSection id="projects" className="mx-auto max-w-7xl px-6 py-32">
          ...
          <Reveal key={title} delay={i * 0.045}>
            <GlassCard className="shine group rounded-[2.7rem] p-4">
              <div className="relative aspect-video overflow-hidden rounded-[2.1rem] bg-[linear-gradient(135deg,#edf7ff,#fff_42%,#dce8f5)]">
                ...
                <span className="absolute left-6 top-6 rounded-full border border-white/70 bg-white/24 px-4 py-2 text-sm font-semibold tracking-[0.25em] text-slate-500 backdrop-blur-2xl">{no}</span>
                <button className="absolute left-1/2 top-1/2 grid size-16 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full bg-white/34 text-xl text-slate-950 opacity-0 shadow-glass backdrop-blur-2xl transition duration-500 group-hover:opacity-100" aria-label="播放视频">▶</button>
              </div>
